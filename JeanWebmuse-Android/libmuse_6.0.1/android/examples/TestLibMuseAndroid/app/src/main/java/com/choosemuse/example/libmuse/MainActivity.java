...
import com.choosemuse.example.libmuse2.R;
...
public class MainActivity extends Activity implements OnClickListener {
    ...
    private final double[] alphaBuffer = new double[6]; //other buffer not included in original example
    ...
    DatabaseReference alphaRef = database.getReference("Device/Alpha_relative");
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.refresh) {
            ...
            if (availableMuses.size() < 1 || musesSpinner.getAdapter().getCount() < 1) {
                ...
                //###############################################################################
                DeviceIdRef.setValue(muse.getName() + " - " + muse.getMacAddress());//set value in database
            ...
            if (muse != null) {
                ...
                //#########
                pausedRef.setValue(String.valueOf(!dataTransmission));
    ...
    public void receiveMuseDataPacket(final MuseDataPacket p, final Muse muse) {
        ...
        switch (p.packetType()) {
            ...
            case BATTERY://add later if you wish
            case DRL_REF:
            case QUANTIZATION:
            default:
                break;
     ...
     * from the buffers.
     DatabaseReference alphaRef = database.getReference("Alpha_relative");
     DatabaseReference betaRef = database.getReference("Beta_relative");
     DatabaseReference thetaRef = database.getReference("Theta_relative");
     DatabaseReference deltaRef = database.getReference("Delta_relative");
     DatabaseReference gammaRef = database.getReference("Gamma_relative");
     */
...
//##########################UPDATE VALUES####################################################
    private void updateEeg() {
        ...
        eegDataRef.child("elem1").setValue(String.format("%6.2f", eegBuffer[0]));// update the child elemnt of the EEG data ref in the datbase
        fp1.setText(String.format("%6.2f", eegBuffer[1]));
        eegDataRef.child("elem2").setValue(String.format("%6.2f", eegBuffer[1]));
        fp2.setText(String.format("%6.2f", eegBuffer[2]));
        eegDataRef.child("elem3").setValue(String.format("%6.2f", eegBuffer[2]));
        tp10.setText(String.format("%6.2f", eegBuffer[3]));
        eegDataRef.child("elem4").setValue(String.format("%6.2f", eegBuffer[3]));
    }
